...
class OnlineIsolationForest(AnomalyDetector):
    ...
    def __init__(self, schema: Schema = None, random_seed: int = 1, num_trees: int = 32, max_leaf_samples: int = 32,
        ...
        self.trees: list[OnlineIsolationTree] = [OnlineIsolationTree.create('boundedrandomprojectiononlineitree',
    ...
    def train_batch(self, data: ndarray):
...
class OnlineIsolationNode(ABC):
    @staticmethod
    def create(inode_type: str, **kwargs) -> 'OnlineIsolationNode':
...
class OnlineIsolationTree(ABC):
    @staticmethod
    def create(itree_type: str, **kwargs) -> 'OnlineIsolationTree':
    ...
    @abstractmethod
    def recursive_build(self, data: ndarray, depths: ndarray[float], node_index: int) -> (int, OnlineIsolationNode):
        pass
...
class BoundedRandomProjectionOnlineINode(OnlineIsolationNode):
    def __init__(self, data_size: int, children: ndarray[OnlineIsolationNode], depth: int, node_index: int,
...
class BoundedRandomProjectionOnlineITree(OnlineIsolationTree):
    def __init__(self, max_leaf_samples: int, type: str, subsample: float, branching_factor: int, data_size: int,
    ...
    def recursive_build(self, data: ndarray, depth: int = 0, node_index: int = 0) -> (int, OnlineIsolationNode):
        ...
        if data.shape[0] < self.max_leaf_samples*OnlineIsolationTree.get_multiplier(self.type, depth) or depth >= self.depth_limit:
            return node_index + 1, OnlineIsolationNode.create('boundedrandomprojectiononlineinode',
